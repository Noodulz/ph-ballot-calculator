...
if st.button("Calculate"):
    ...
    if ballot_type_option == "Albums":
        if chosen == "Yes":
            result = subprocess.run([sys.executable, "calcs/album_calc_SPOILERED.py", tmp_path, "censor"], capture_output=True, text=True)
        else:
            result = subprocess.run([sys.executable, "calcs/album_calc_NOSPOILER.py", tmp_path], capture_output=True, text=True)
    
    elif ballot_type_option == "All Stars":
        if chosen == "Yes":
            result = subprocess.run([sys.executable, "calcs/allstars_calc_SPOILERED.py", tmp_path, "censor"], capture_output=True, text=True)
        else:
